...
HistObs <- getHistoricalObs(station_id, window = window)
...
dist.plot <- ggplot(data = HistObs, aes(Tavg)) + 
  ...
  annotate("text", x = Tavg.now, y = Inf, vjust = -0.75, hjust = 1.1,
           ...
           family = 'Roboto Condensed', fontface = "bold")
  #########################################################
  ### HOW THE HELL DO YOU DO TEXT OUTSIDE THE PLOT IN R???
  # +labs(caption=paste0("'This time of year' based on a moving ",2*window+1," day window"),size=2)
  #########################################################
