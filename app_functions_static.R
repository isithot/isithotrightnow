...
getHistoricalObs <- function(station_id, date = Sys.Date(), window = 7) {
  ...
  # # Read historical obs
  HistObs.Tmax <- read.table(paste0(fullpath,"data/acorn.sat.maxT.", station_id, ".daily.csv"),
                            header = FALSE, skip = 2,
                            colClasses = c("character","numeric","NULL","NULL"),
                            col.names = c("Date","Tmax","null","null"),
                            na.strings=c(""," ","NA"),
                            sep = ",")
  HistObs.Tmin <- read.table(paste0(fullpath,"data/acorn.sat.minT.", station_id, ".daily.csv"),
                            header = FALSE, skip = 2,
                            colClasses = c("character","numeric","NULL","NULL"),
